				...
				<div class="col-lg-9 order-1 order-lg-2">
					...
					<div class="paginationRight" >
						<div class="pagination">
							...
							<!-- previous -->
							<%if(current>1){ %>
							<a href='<c:url value="/category/${id }?page=${previous }"/>'>&laquo;</a>
							<%}else{%>
							<a href='<c:url value="/category/${id }?page=${currentPage } "/>'>&laquo;</a>
							<%} %>
							<!-- index -->
							...
							<a <%if(current==i){ %>class="active"<%} %>
							 ...
							 <%if(current<totalPage){ %>
  							<a href='<c:url value="/category/${id }?page=${next }"/>'>&raquo;</a>
  							<%}else{%>
  							<a href='<c:url value="/category/${id }?page=${currentPage }"/>'>&raquo;</a>
  							<%} %>
						</div>
					</div>
				</div>
