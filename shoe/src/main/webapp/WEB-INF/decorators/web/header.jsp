		...
		<div class="container">
			<div class="ht-right">
				...
				<security:authorize access="isAnonymous()">
					<a href='<c:url value="/dang-nhap" />' class="login-panel">
		...
		<div class="inner-header">
			<div class="row">
				...
				<div class="col-lg-3 text-right col-md-3">
					<ul class="nav-right">
						<li class="cart-icon"><i class="icon_bag_alt"></i><a class="iconTotalQuantyCart" href="#">  ${TotalQuantyCart } </a>
							<div class="cart-hover">
									...
									<table id="table">
										<tbody id="tableTbodyIconCart">
											<c:forEach var="item" items="${Cart }">
												<tr>
													<td class="si-pic"><img src='<c:url value="/template/web/img/products/${item.value.product.img }" />' alt=""></td>
													<td class="si-text">
														<div class="product-selected">
															<div>${item.value.product.name }</div>
															<div>x${item.value.quanty }</div>
															<div><fmt:formatNumber pattern="#,##0 vnđ" value="${item.value.totalPrice }" /></div>
														</div>
													</td>
													<td class="si-close"><a onclick="deleteCart(${item.key })"><i class="ti-close"></i></a></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
								...
								<div class="select-total">
									<span>total:</span>
									<h5 class="totalPriceIconCart"><fmt:formatNumber pattern="#,##0 vnđ" value="${TotalPriceCart }" /></h5>
								</div>
						...
						<li class="cart-price"><fmt:formatNumber pattern="#,##0 vnđ" value="${TotalPriceCart }" /></li>
					</ul>
				</div>
				<!-- Gio Hang End -->
			</div>
